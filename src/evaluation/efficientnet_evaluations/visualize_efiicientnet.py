"""
visualize.py — Evaluation Visualiser for EfficientNet-B3
Reads:
  - src/evaluation/eval_report.json              (required — written by evaluate.py)
  - src/models/efficientnet/training_log.json    (optional — written by train_efficientnet.py)
...
Run from project root:
    python3 src/models/efficientnet/visualize.py
"""
...
# ── Config ─────────────────────────────────────────────────────────────────────
LOG_PATH        = "src/models/efficientnet/training_log.json"
...
PHASE_A_EPOCHS  = 5      # must match train_efficientnet.py
PALETTE         = {
    "train":     "#4C9BE8",
    "val":       "#E87D4C",
    "phaseA":    "#C8E6C9",
...
def plot_training_curves(log: dict):
    ...
    fig.suptitle("EfficientNet-B3 — Training Curves", fontsize=15, fontweight="bold", y=1.01)
...
def plot_per_class_metrics(report: dict):
    ...
    ax.set_title(
        f"Per-Class Metrics — EfficientNet-B3  |  "
...
def plot_confusion_matrix(report: dict):
    ...
    ax.set_title("Confusion Matrix — EfficientNet-B3", fontsize=13, fontweight="bold")
...
def plot_summary_card(report: dict, log: dict | None = None):
    ...
    lines = [
        ("EfficientNet-B3 — Evaluation Summary", 0.92, 16, "#CDD6F4", "bold"),
...
def main():
    ...
    if os.path.exists(LOG_PATH):
        ...
        print(f"         To generate training_curves.png, run train_efficientnet.py first.\n")
